...
class Loader {
  ...
  render() {
    clearTransform(new Vector4(float2int(resolution.x / 2), float2int(resolution.y / 2), resolution.x, resolution.y), 0); layers[0].context.font = "400px Monaco, monospace"; layers[0].context.textBaseline = "middle"; layers[0].context.textAlign = "center";
    layers[0].context.fillText(`${float2int(this.progress / this.images_count * 100)}%`, float2int(resolution.x / 2), float2int(resolution.y / 2));
  }
}
...
class SceneControl extends GameObject {
  ...
  clearObjects() { for (let i = 2; i < objects.length; i++) { objects[i].destroyed = true; } for (let i = 0; i < layers.length; i++) clearTransform(this.transform, i); }
